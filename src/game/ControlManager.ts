module control
{
    export class ControlBasic{
        protected stage:moon.BasicGamePanel;
        ...
        constructor(stage:moon.BasicGamePanel) {
            this.stage=stage;
        }
        /** 打开事件*/
        public open():void{
    ...
    export class ControlBarMove extends ControlBasic{
        ...
        constructor(stage:moon.BasicGamePanel,controlBar:egret.DisplayObject,controlBg:egret.DisplayObject) {
            super(stage);
    ...
    export class ControlDrag extends ControlBasic{
        ...
        isDrag:boolean;
        constructor(stage:moon.BasicGamePanel,display:egret.DisplayObject) {
            super(stage);
    ...
    export class ControlMoreTab extends ControlBasic{
        ...
        constructor(stage:moon.BasicGamePanel,datas:any[]) {
            super(stage);
    ...
    export class SequenceRandom{
        public static randomSequence():Array<number>{
            var squence = GameData.gameType;
            squence = SequenceRandom.shuffter(squence);
            return(squence);
        }

        public static shuffter(squence:Array<number>){
            for(let j_=squence.length-1; j_>=0; j_--){
                let j = Math.floor(Math.random() * (j_));
                let ith_array_first = squence[j_];
                let ith_array_second = squence[j];
                if(Math.random()>=0.5) {
                    squence[j] = ith_array_first;
                    squence[j_] = ith_array_second;
                }
            }
            return squence;
        }
