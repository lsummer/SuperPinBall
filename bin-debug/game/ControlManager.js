...
(function (control) {
    ...
    var SequenceRandom = (function () {
        ...
        SequenceRandom.shuffter = function (squence) {
            for (var j_ = squence.length - 1; j_ >= 0; j_--) {
                var j = Math.floor(Math.random() * (j_));
                var ith_array_first = squence[j_];
                var ith_array_second = squence[j];
                if (Math.random() >= 0.5) {
    ...
    control.SequenceRandom = SequenceRandom;
    __reflect(SequenceRandom.prototype, "control.SequenceRandom");
})(control || (control = {}));
